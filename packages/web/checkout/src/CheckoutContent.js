...
import { getProductImage } from 'search/ProductService'

import { checkout } from './CartService'
    ...
    <tbody>
      {items.map(({ pokemon, count }) => (
        <tr key={pokemon.name.english}>
          <td width="5%">
            <img src={getProductImage(pokemon)} style={{ maxHeight: 50 }} />
          </td>
      ...
      <tr>
        ...
        <td id="total">${items.reduce((a, { count, pokemon: { price } }) => a + count * price, 0)}</td>
      </tr>
    </tbody>
  </Table>
)
